...
class AccountControllerIntegrationTests {
	...
	private Account setUpAccount() {
		...
		if(account == null) {
			account = new Account();
			account.setUsername("theMusician");
			account.setEmail("violinsrock@gmail.com");
	...
	public void login_success() throws Exception {
		...
		String json = gson.toJson(setUpLogin(account.getEmail(), accountPassword));
	...
	public void login_fail_password() throws Exception {
		Account account = setUpAccount();
		String json = gson.toJson(setUpLogin(account.getEmail(), "not" + accountPassword));
		mockMvc.perform(post("/accounts/login").contentType(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isUnauthorized())
				.andExpect(jsonPath("$.description").value("The username or password is incorrect"));
		deleteAccount();
